<?php
class Gala_Rainbowsettings_Model_Observer {
	
	public function beforeGenerateBlocks(Varien_Event_Observer $observer) {
		if((Mage::getSingleton('core/design_package')->getPackageName() == 'default')&& (Mage::getDesign()->getTheme('frontend') == 'galarainbow')){	
			# Disable default magento navigation
			if (Mage::helper('rainbowsettings')->getGeneral_DisableDefaultNav()&&(Mage::getConfig()->getModuleConfig('EM_Megamenupro')->is('active', 'true'))) {
				$blocks = $observer->getLayout()->getXpath('//block[@name="galarainbow.catalog.topnav"]');
				if (!empty($blocks))
					$blocks[0]->addAttribute('ignore', true);
                $blocks = $observer->getLayout()->getXpath('//block[@name="galarainbow.catalog.topnav.left"]');
				if (!empty($blocks))
					$blocks[0]->addAttribute('ignore', true);
			}
		
			# Disable EM variation module on frontend
			if (Mage::helper('rainbowsettings')->getGeneral_DisableFrontendVariation()) {
			...
			if (Mage::helper('rainbowsettings')->getGeneral_DisableFooterLinks()) {
	...
	public function beforeCatalogProductCollectionLoad(Varien_Event_Observer $observer) {
		if((Mage::getSingleton('core/design_package')->getPackageName() == 'default')&& (Mage::getDesign()->getTheme('frontend') == 'galarainbow')){		
			//$alt_img = Mage::helper('rainbowsettings')->getProductsGrid_AltImg();
			//if ($alt_img == 'image')
