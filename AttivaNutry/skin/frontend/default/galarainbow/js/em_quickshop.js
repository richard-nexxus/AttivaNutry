...
jQuery(function ($) {
	...
	var _qsJnit = function() {
		...
		var _qsHref = "<a id=\"em_quickshop_handler\" href=\"#\" style=\"visibility:hidden;position:absolute;top:0;left:0\" title=\""+EM.QuickShop.QS_TEXT+"\"><span><span>"+EM.QuickShop.QS_TEXT+"</span></span></a>";
			...
			if(($(value).parents(".no_quickshop").length <= 0)&&(isMobile==false)){
				...
				$(selectorObj.imgClass, this).bind('mouseover', function () {
					...
					$('#em_quickshop_handler').attr('href', reloadurl).show().css({
							'top': o.top+($(this).height() - EM.QuickShop.QS_BTN_HEIGHT - 20)/2+'px',
							'left': o.left+($(this).width() - EM.QuickShop.QS_BTN_WIDTH + 60)/2+'px',
							'visibility': 'visible',
							'z-index':	999
					});
				});
